...
-- | SideBorderLayout but takes takes a theme and a direction
sideBorderLayout' :: Eq a => Theme -> Direction2D -> l a -> SideBorder l a
sideBorderLayout' theme sbSide =
  decoration BorderShrinker deco (SideBorderDecoration sbSide)
 where
  deco | sbSide `elem` [U, D] = theme{ decoHeight = activeBorderWidth theme }
       | otherwise            = theme{ decoWidth  = activeBorderWidth theme }
